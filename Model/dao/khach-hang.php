...
function user_login($username, $password){
    return pdo_query_one("SELECT * FROM users Where username = '$username' and password = md5('$password')");
}

function  user_checkUsername($username){
    return pdo_query_one("SELECT * FROM users Where username =?",$username);
}
   function user_register($username, $email,  $password){
    return pdo_execute("INSERT INTO `users`( `username`, `email`,`password`) VALUES ('$username', '$email',  md5('$password'))");
}
function user_getById($id_user){
    return pdo_query_one("SELECT * from users where id_user=?", $id_user);
}